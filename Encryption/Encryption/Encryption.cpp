...
{
    unsigned char* byte_array = hexstr_to_char(sw::sha512::calculate(password).c_str());
    return byte_array;
}
...
char* pass_bytes_through_gates(uint8_t* gates, char* data_bytes, bool encoding)
{
    for (int i = 0; i < 24; i++)
    {
        data_bytes_seperated[i][0] = data_bytes[i * 3];
        data_bytes_seperated[i][1] = data_bytes[i * 3 + 1];
        data_bytes_seperated[i][2] = data_bytes[i * 3 + 2];
    }
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
            return_bytes_buffer[i * 3 + j] = new_data_bytes_seperated[i][j];
    }
...
void XOR(char* arr1, char* arr2, char amount)
{
    for (int i = 0; i < 72; i++)
        *&arr1[i] ^= *&arr2[(i+amount)%72];
}
...
{ //Dont complain about the lack of comments if its such an issue dont do it
    ...
    bool encrypting = false; //make sure this is false if you are trying to decrypt the file. or you will overwrite the encrypted file
    bool delete_old_file = false; //this does nothing just now
    char security_level = 1; //Level of security
    std::string first_file = "first_file.txt"; //Unencrypted File Name
    std::string second_file = "second_file.out"; //Encrypted File Name
    ...
    int block_amount = -1;
    int bytes_read = 0;
    int total_blocks = encrypting ? ceil((length + 1) / 72.0f) : (length / 72);
    char extra_data = encrypting ? 72 - (length % 72) : 0;
    std::cout << +extra_data << std::endl;
    size_t buffer_size = fmin(65536, total_blocks) * 72;
    char* buffer = new char[buffer_size];
    std::cout << "Total Blocks: " << total_blocks << ", Extra Data: " << +extra_data << std::endl;
    ...
    {
        bytes_file.read(buffer, buffer_size);
        ...
        if (encrypting)
        {
            if(block_amount == 0 || block_amount == total_blocks - 1)
                XOR(sentence, (char*)lHash, 0);
            else
                XOR(sentence, last_sentence, 0);
            for (int i = 0; i < 4; i++) 
                for (int j = 0; j < keys.bitKeyPermutationAmounts[i] % (int)pow(2, security_level); j++)
                {
