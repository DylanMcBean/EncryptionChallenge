...
int main(int argc, char** argv)
{
	...
	keys = get_gates(generate_key(argv[1])); //generate keys from password
	...
	while (block_amount < (total_blocks - 1))
	{
		...
		if (encrypting)
		{
			...
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < fmax(1,keys.bitKeyPermutationAmounts[i] % (1 << security_level)); j++)
				{
		...
		if (!encrypting && block_amount == 0)
		{
			...
			{
				fwrite(sentence + 17, 1, fmin(72, 72 - (total_blocks == 1 ? extra_data : extra_data - 17)), append_file);
			}
			else
			{
				fwrite(sentence + 17, 1, 55, append_file);
			}
		}
		else if (!encrypting && block_amount == total_blocks - 1)
		{
			fwrite(sentence, 1, fmin(72, 72 - (extra_data - 17)), append_file);
		}
		else
		{
